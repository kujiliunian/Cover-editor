...
export class MLeaferCanvas {
    ...
    constructor(
        ...
        const myCustomTheme = {
            ...
            textColor: '#fbfbfb20',      // 白色文字
            borderColor: '#555555',     // 刻度线颜色
            highlightColor: '#00FFCC10'  // 高亮颜色 (例如鼠标位置的指示线)
        };

        this.ruler.addTheme('myCustomTheme', myCustomTheme);

        this.ruler.changeTheme('myCustomTheme');
    ...
    initPageEditor() {
        ...
        this.app.editor.on(EditorEvent.SELECT, (arg: EditorEvent) => {
            // ================== 核心修改在这里 (最终方案) ==================
            /**
             * 使用 nextTick (或 setTimeout) 将配置修改推迟到下一个微任务/宏任务。
             * 这可以确保我们的修改发生在 Leafer Editor 内部所有与按钮相关的
             * 初始化逻辑之后，从而避免被覆盖。
             */
            nextTick(() => {
                if (this.app.editor && this.app.editor.config) {

                    // 为了最终确认，我们在这里打印一下，它应该能被执行
                     console.log(this.app.editor.config.buttons);

                    this.app.editor.config.buttons = [
                        { name: 'clone' },   // 保留克隆（复制）按钮
                        { name: 'remove' }   // 保留删除按钮
                    ];

                    // Leafer 可能需要一次强制更新来应用这个配置变化
                    if (this.app.editor.element) {
