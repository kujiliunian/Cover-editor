...
export const useActiveObjectModel = <K extends keyof ILeaf, T = ILeaf[K] | undefined>(
    ...
    const setValue = (newValue: T) => {
        // 检查 editor.contentFrame 是否存在
        if (!isDefined(editor.contentFrame)) {
            console.warn('Cannot set value: editor.contentFrame is not defined.');
            return;
        }
        // 直接调用 setObjectValue，并把 editor.contentFrame 作为目标对象
        setObjectValue(editor.contentFrame, newValue);
    }
    return computed(() => ({
