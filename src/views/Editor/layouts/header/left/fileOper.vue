        ...
        <template #content>
            <div>
                <div class="p-t-5px p-b-5px p-l-20px p-r-20px border-bottom">
                    <span style="color: #1e2025;font-weight: bold">ID:{{ wordID }}</span>
                    <div>
                        <span style="color: #1e2025;font-weight: bold">当前项目：{{ wordname }}</span>
                    </div>
                </div>
                <ul class="">
  ...
  if(route.query.id){
    ...
    getProjectInfo(route.query.id).then( res=> {
      if (res.code == 200){
        ...
        //   visible.value = false
        // };
        wordname.value=res.data.project_name
      }
    }).catch( err=>{
